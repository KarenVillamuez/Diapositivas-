...
Fuentes:
  * logs/benchmarks/*: cuatro semillas por modelo para cfg_eval_full.
  * anclaje-docente/captura_docentes.csv: valoraciones de los dos docentes.

Salidas:
  * SVG vectorial en la raíz del artículo.
  * PNG a 600 DPI y PDF vectorial en ``figuras/``.
"""
...
RUN_FILES = {
    "Phi-4-mini": dict(
        zip(
            SEEDS,
            (
                "benchmark_20260902_140238.json",
                "benchmark_20260903_122324.json",
                "benchmark_20260903_125347.json",
                "benchmark_20260903_131956.json",
            ),
        )
    ),
    "Qwen2.5-3B": dict(
        zip(
            SEEDS,
            (
                "benchmark_20260902_144618.json",
                "benchmark_20260903_123651.json",
                "benchmark_20260903_130631.json",
                "benchmark_20260903_135021.json",
            ),
        )
    ),
}
...
def comma_decimal(value: float, _position: int | None = None) -> str:
...
def load_run(model: str, filename: str) -> dict[str, dict]:
    path = BENCHMARK_DIR / MODEL_DIRS[model] / filename
    with path.open(encoding="utf-8") as handle:
        payload = json.load(handle)
    rows = payload["results"] if isinstance(payload, dict) else payload
    selected = {row["question_id"]: row for row in rows if row["config_id"] == "cfg_eval_full"}
    missing = set(CONTENT_QUESTIONS) - set(selected)
    if missing:
        raise ValueError(f"Faltan preguntas en {path}: {sorted(missing)}")
    return selected


def load_benchmark_data() -> dict[str, dict[str, dict[str, dict]]]:
    return {
        model: {seed: load_run(model, filename) for seed, filename in RUN_FILES[model].items()}
        for model in MODELS
    }
...
def load_teacher_data() -> dict[str, dict[str, tuple[int, int, int]]]:
    ...
    source = PAPER_ROOT / "anclaje-docente" / "captura_docentes.csv"
    with source.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            teacher = "Docente 1" if row["evaluador"] == "docente_1" else "Docente 2"
            ratings[teacher][row["codigo"]] = tuple(
                int(row[column]) for column in ("correccion", "claridad", "adecuacion_nivel")
            )
...
def figure_01_score_anatomy(overwrite: bool) -> dict[str, Path]:
    fig, axis = plt.subplots(figsize=(7.08, 2.75))
    segments = (
...
def figure_02_score_by_question(data, overwrite: bool) -> dict[str, Path]:
    fig, axes = plt.subplots(1, 2, figsize=(7.08, 3.35), sharey=True)
    for axis, model in zip(axes, MODELS):
        ...
        axis.set_ylim(0, 1.0)
        axis.yaxis.set_major_formatter(FuncFormatter(comma_decimal))
...
def figure_03_variation(data, overwrite: bool) -> dict[str, Path]:
    ...
    score_axis.yaxis.set_major_formatter(FuncFormatter(comma_decimal))
...
def figure_04_teacher_ratings(ratings, overwrite: bool) -> dict[str, Path]:
    ...
    fig, axis = plt.subplots(figsize=(7.08, 3.65))
    ...
    axis.text(
        0,
        1.07,
        "Cada línea representa una respuesta. El piloto no estableció acuerdo suficiente\n"
        "(rho de Spearman = −0,50; kappa de Cohen = −0,43 para el uso sin modificación).",
        transform=axis.transAxes,
        fontsize=7.2,
        color=MUTED,
        linespacing=1.35,
