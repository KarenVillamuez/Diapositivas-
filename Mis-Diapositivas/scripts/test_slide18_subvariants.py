...
# We compare:
# Subvariant B1-A: Left box: Tag (15px mono) + APORTE CENTRAL (22px mono)
# Subvariant B1-B: Left box: Tag (15px mono) + Tesis Defendida (28px sans bold) + APORTE CIENTÍFICO (13px mono)
# Subvariant B1-C: Left box: Badge vino estilizado (16px mono bold) con icono o subtítulo fino

def generate(left_html, bold_thesis=False):
    thesis_text = (
  ...
  <div class="slide-content-area" style="top: 175px; height: 720px; justify-content: space-between;">
    ...
    <div style="background: #F4E8EA; border-left: 8px solid var(--c-wine-primary); padding: 18px 36px; display: grid; grid-template-columns: 290px 1fr; gap: 32px; align-items: center; border-radius: 2px; border-top: 1px solid rgba(107, 29, 47, 0.12); border-right: 1px solid rgba(107, 29, 47, 0.12); border-bottom: 1px solid rgba(107, 29, 47, 0.12);">
      <div style="border-right: 2px solid #D8B8BE; padding-right: 24px; text-align: center;">
        {left_html}
      </div>
...
def main():
    ...
    left_2 = '''
      <div style="font-family: var(--font-mono); font-size: 14px; font-weight: 800; color: #5D4A4D; letter-spacing: 1.5px; margin-bottom: 2px;">
        CONCLUSIÓN GLOBAL
      </div>
      <div style="font-family: var(--font-sans); font-size: 28px; font-weight: 900; color: var(--c-wine-primary); line-height: 1.1;">
        Tesis Defendida
      </div>
    '''
    ...
    with sync_playwright() as p:
        ...
        for name, html_content in [
            ("slide18_sub_b1", generate(left_1, bold_thesis=True)),
            ("slide18_sub_b2", generate(left_2, bold_thesis=True)),
        ]:
            html = HTML_PAGE.format(content=html_content)
